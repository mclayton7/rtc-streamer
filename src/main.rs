...
async fn main() -> anyhow::Result<()> {
    ...
    tokio::spawn(async move {
        metrics_clone.start_periodic_reporting().await;
    });

    // Single channel: UDP receiver → demuxer (RTP depacketization is inlined)
    let (udp_tx, udp_rx) = mpsc::channel(100);
    ...
    let signaling_server = SignalingServer::new(config.signaling.clone(), session_manager);
